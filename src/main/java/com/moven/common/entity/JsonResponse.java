package com.moven.common.entity;
 ...
 * <p>Description:Json返回格式约定类</p>
 * @author Shengwei
 * @date 2016年12月1日 下午2:45:07
 */
public class JsonResponse implements Serializable {
	 ...
	 * 则后续不能在添加不符合json规范的类型。如： add(list).add(map), add(list).add(number)都会抛出异常。
	 * 但 add(list).add(list), add(map).add(map)不会抛出异常。
	 * 
	 * @param bean
	 *            bean
	 * @return JsonResponse
		...
		if (bean instanceof Map) {
			...
			if (EMPTY == data) {
				...
				throw new IllegalStateException(
						"不能将" + bean.getClass().getSimpleName() + "类型追加到" + data.getClass().getSimpleName() + "类型");
			}
